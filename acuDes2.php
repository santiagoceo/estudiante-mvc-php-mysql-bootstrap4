<?php
    ...
    $datosMunicipio = $objMunicipio->consultar($conexion);
...
<div class="d-flex flex-column flex-md-row align-items-center p-3 px-md-4 mb-3 bg-white border-bottom shadow-sm">
    ...
    <div class="container text-center">
        ...
        <p class="lead">Para eliminar un acudiente verifica la información y haz clic en eliminar</p>
    </div>
    <div class="container pb-3">
        <div class="row">
        <div class="col"></div>
            <div class="col">
            <form action="controlador/acuDes.php" method="post">
                <div class="form-group text-center">
                    <label for="Nombre">Nombre</label>
                    <input type="hidden" class="form-control" name="id" value="<?php echo $id ?>" required>
                    <input type="text" class="form-control" placeholder="Nombre" name="nombre" value="<?php echo $nombre ?>" required disabled>
                </div>
                <div class="form-group text-center">
                    <label for="Apellido">Apellido</label>
                    <input type="text" class="form-control" placeholder="Apellido" name="apellido" value="<?php echo $apellido ?>" required disabled>
                </div>
                <div class="form-group text-center">
                    <label for="Fecha de nacimiento">Fecha de nacimiento</label>
                    <input type="date" class="form-control" placeholder="Fecha de nacimiento" name="fechaNacimiento" value="<?php echo $fechaNacimiento ?>" required disabled>
                </div>
                <div class="form-group text-center">
                    <label for="Teléfono">Teléfono</label>
                    <input type="text" class="form-control" placeholder="Teléfono" name="telefono" value="<?php echo $tel ?>" required disabled>
                </div>
                <div class="form-group text-center">
                    <label for="Dirección">Dirección</label>
                    <input type="text" class="form-control" placeholder="Dirección" name="direccion" value="<?php echo $direccion ?>" required disabled>
                </div>
                <div class="form-group text-center">
                    <label for="Municipio">Municipio</label>
                    <select class="form-control" id="exampleFormControlSelect1" name="idMunicipio" disabled>
                        <?php
                        while($data = mysqli_fetch_array($datosMunicipio)){
                        ?>
                        <option value="<?php echo $data['id_municipio'] ?>" 
                        <?php 
                        if ($data['id_municipio']==$id_municipio){
                        ?>
                        selected 
                        <?php
                        }
                ...
                <button type="submit" class="btn btn-primary">Eliminar acudiente</button>
                </div>
            </form>
            </div>
        <div class="col"></div>
